package com.xiaozhan.micrpay.service;
...
import com.xiaozhan.micrpay.config.AlipayConfig;
...
public class AlipayService {
    ...
    public String alipayPagePay(Integer uid,BigDecimal money,String channel){
        ...
        record.setRechargeStatus(CommonContants.INCOME_STATUS_PROCESSING);
        ...
        if( rows > 0 ){
            ...
            if(form!=null&&form.length()>0){
                //把订单号和充值时间，存放到redis的ZSet中
                stringRedisTemplate.opsForZSet().add(RedisKey.ALIPAY_ALL_TRANDE_NO,record.getRechargeNo(),record.getRechargeTime().getTime());
            }
